...
const clientsStore = useGlobalStore('clients')
...
const data = computed(() => {
    return clientsStore.getData()
})

// Watch clientStore while is fetching
const unwatch = watch(() => clientsStore.fetching, () => {
    clientsStore.fetching && apiGrid.value.showLoadingOverlay()

    !clientsStore.fetching && apiGrid.value.hideOverlay()
