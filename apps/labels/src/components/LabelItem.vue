...
const labelElementReactive = reactive(props.modelValue)
...
const qrCodeIsSelected = labelElementReactive.name === qrCode ? ref(true) : ref(false)
...
const elementChange = (event) => {
    ...
    labelElementReactive.name = selectedElement?.name
    labelElementReactive.label = selectedElement?.label
    labelElementReactive.field = selectedElement?.field
    labelElementReactive.type = selectedElement?.type

    if (selectedElement.id !== 'free_text') {
        labelElementReactive.text = ''
    }
...
const toggleBold = () => {
    labelElementReactive.is_bold = !labelElementReactive.is_bold
}

// Handles changes in the labelElementReactive object
watch(
    () => labelElementReactive,
    () => { elementChange(labelElementReactive.element_id) },
    { deep: true }
)
...
<template>
    ...
    <div class="grid md:grid-cols-12 gap-3">
        ...
        <button
            ...
            type="button"
            color="primary"
            class="button hover:delete hover:outline w-20 md:col-span-1 h-min">
            <TrashIcon
