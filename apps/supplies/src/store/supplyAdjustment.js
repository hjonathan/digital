...
export default defineStore('supplyAdjustment', () => {
    const data = ref([])
    const request = ref()
    const schema = ref([])
    ...
    const fetchRequests = async () => {
        ...
        const response = await api.post('/supplies/adjutment_list', {})
    ...
    const fetchRequestById = async (id) => {
        ...
        const response = await api.get(`/supplies/adjutment_show/${id}`)
