...
const useMaterialAssign = useComposable({ useGlobalStore, configSchema })
...
const { data, variables } = useMaterialAssign
...
onMounted(async () => {
    if (id) {
        const response = await api.get(`supplies/material_assignment_show/${id}`)
