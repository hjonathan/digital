...
import DispatchNoteMA from './DispatchNoteMA.vue'
...
const buttons = configButtons({ useAlertModal, useDispatchNote, useGlobalStore, configModal })
...
type.value = 'material-assignment'

onMounted(async () => {
    if (id) {
        const response = await api.get(`supplies/material_assignment_show/${variables.value.supplyOrderId}`)
...
<template>
    <Overlay v-if="overlay" class="z-50"/>
