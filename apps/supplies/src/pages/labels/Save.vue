...
import { ref, reactive, shallowReactive, onMounted, inject, computed } from 'vue'
...
const elements = [
    ...
    },
    {
        ...
        name: t('Category'),
        type: 'text',
        example: t('Office supplies')
    },
    {
        id: 'location.slug',
        name: t('Location'),
        type: 'text',
        example: t('Shelving area')
    },
    {
        id: 'measure.slug',
...
const save = async (button) => {
    ...
    if (labelId && mode) {
        ...
        response.success && labelsStore.fetch() && tabsStore.closeTab('SuppliesLabelsClone')
    }
    ...
    if (labelId && !mode) {
        ...
        response.success && labelsStore.fetch() && tabsStore.closeTab('SuppliesLabelsEdit')
    }
    ...
    if (!labelId) {
        ...
        /*         const res = await axios.post('http://127.0.0.1:8000/api/labels', label)

        response = res.data */

        response.success && labelsStore.fetch() && tabsStore.closeTab('SuppliesLabelsCreate')
    }
    ...
    rapidStore.$emitGlobalEvent('resetLabelsSelected')
