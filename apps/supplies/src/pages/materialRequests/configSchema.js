...
export const configSchema = ({ useGlobalStore, data, create }) => {
    ...
    // MANAGER
    if (jsonLogic.apply(getRules(dataRules).manager, dataRules)) {
        return {
            role: getRole(dataRules),
            document: {
                title: 'Material request'
    ...
    // WAREHOUSE
    if (jsonLogic.apply(getRules(dataRules).warehouse, dataRules)) {
        return {
            role: getRole(dataRules),
            document: {
                title: 'Material request'
...
const getRole = (dataRules) => {
    ...
    if (jsonLogic.apply(getRules(dataRules).warehouse, dataRules)) { return 'warehouse' }
