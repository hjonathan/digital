...
onMounted(() => {
    ...
    for (const item of purchaseOrder.value.supply) {
        if (!item.partsItems) item.partsItems = []
    }
...
const getColumns = computed(() => {
    const columns = [
        { field: 'request_user.name', name: 'Receive parts', align: 'left', customClass: '!w-1/12' },
        {
            field: 'request_user.name',
            name: t('Name'),
            align: 'left',
            customClass: '!w-4/12'
        },
...
const addPartialItem = (index) => {
    purchaseOrder.value?.supply[index]?.partsItems?.push({})
}

const removeItemPartial = (index, indexPartial) => {
    purchaseOrder.value.supply[index].partsItems.splice(indexPartial, 1)
}
...
const openPartReception = (nValue, index, toggleExpansion) => {
    ...
    if (!purchaseOrder.value?.supply[index]?.partsItems.length) purchaseOrder.value?.supply[index]?.partsItems?.push({})
}
...
<template>
            ...
            <div
                ...
                <TableUI
                    ...
                    <template v-slot:row="{ row, index, toggleExpansion }">
                        <tr :class="{ 'border-b border-gray-100 print-custom-leading': true, 'bg-gray-200': row.isOpen, 'bg-red-100':verifyErrorRow(row, index) }">
                            <!-- Name -->
                                ...
                                <TableUI
                                    ...
                                    <template v-slot:row="{ row, index: indexPartial, toggleExpansion }">
                                        ...
                                        <td class="!w-2/12 px-5">
                                            ...
                                            <Input
                                                v-model="row.name"
                                                :errors="errors && errors[`order_supplies.${index}.partsItems.${indexPartial}.name`] ? [] : null"
                                                :placeholder="$t('Name')"
                                                container-class="none" />
                                        </td>
                                        ...
                                        <td class="!w-3/12 px-5">
                                            ...
                                            <Input
                                                v-model="row.description"
                                                :errors="errors && errors[`order_supplies.${index}.partsItems.${indexPartial}.description`] ? [] : null"
                                                :readonly="mode === 'readonly'"
                                                :placeholder="$t('Description')"
                                                container-class="none" />
                                        </td>
                                        ...
                                        <td class="!w-2/12 px-5">
                                            ...
                                            <Input
                                                v-model="row.quantity"
                                                :errors="errors && errors[`order_supplies.${index}.partsItems.${indexPartial}.quantity`] ? [] : null"
                                                :readonly="mode === 'readonly'"
                                                :inputClass="'text-right'"
                                                type="number"
                                                min="0"
                                        ...
                                        <td class="!w-2/12 px-5">
                                            ...
                                            <Treeselect
                                                v-model="row.measure_id"
                                                :errors="errors && errors[`order_supplies.${index}.partsItems.${indexPartial}.measure_id`] ? [] : null"
                                                :readonly="mode === 'readonly'"
                                                :options="measures"
                                        ...
                                        <td class="!w-3/12 px-5">
                                            ...
                                            <Treeselect
                                                v-model="row.location_id"
                                                :errors="errors && errors[`order_supplies.${index}.partsItems.${indexPartial}.location_id`] ? [] : null"
                                                :readonly="mode === 'readonly'"
                                                :options="warehouseLocations"
    ...
    <div
        ...
        <table class="table-without-borders w-full">
            ...
            <tbody>
                <template
                    v-for="(order, index) in purchaseOrder.supply?.filter(item => !item.partsItems.length)"
                    :key="index">
                    ...
                    <tr v-if="order.combinedItems.length">
                        <td colspan="100%"
                            class="pl-20">
                            <Title
                                :title="$t('Parts')"
                            ...
                            <table>
                                <thead class="bg-gray-200 border-b border-t border-b-gray-300 border-t-gray-200">
                                    <tr>
                                        <!-- Name -->
                                        <th class="w-[10%] font-semibold text-start pl-4 py-2">
                                            {{ $t('Name') }}
                                        </th>
                                ...
                                <tbody>
                                    <template
                                        v-for="(part, indexPart) of order.combinedItems"
                                        :key="indexPart">
                                        <tr>
                                            <td class="pl-5">{{  part.name }}</td>
                                            <td>{{  part.description }}</td>
                                            <td>{{  part.quantity }}</td>
                                            <td>{{ getMeasureByID(part.measure_id)?.data?.name }}</td>
                                            <td>{{ getLocationByID(part.location_id)?.data?.name }}</td>
                                        </tr>
                                    </template>
                                </tbody>
                            </table>
