...
const fields = ref([
    {
        name: t('Receipt ID'),
        field: (row) => {
            return showUuid(row.receipt_id)
        }
    },
    {
        name: t('ID'),
    ...
    },
    {
        name: t('Name'),
        field: (row) => {
            return showUuid(row.name)
        }
    },
    {
        name: t('Quantity'),
        field: (row) => {
            return `${row.quantity} ${parametersStore.getItemById(row.measure_id).name}`
        }
    },
])
...
<template>
    ...
    <div class="py-4 text-sm">
        <template v-for="(row, rowIndex) in model" :key="rowIndex">
            <div :class="['detail-receipt grid grid-cols-2 !pt-4 !pb-4 !pl-4 !pr-4', {'bg-gray-100':rowIndex%2 == 1}]">
                <div>
