...
const steps = ref([
    {
        id: 'items_result',
        name: t('Hang Weight'),
        subtitle: t('Introduce Hang Weight'),
        status: 'current',
        prefix: 'Hang Weight',
        complete: true,
        optional: false,
        ...
        errors: [],
        next: async (goNext) => {
            const response = await validateStep('items_result', 'stock/processing_validation')
    ...
    },
    {
        ...
        errors: [],
        next: async (goNext) => {
            const response = await validateStep('waste', 'stock/processing_validation')
...
const save = async () => {
    ...
    const responseLastStep = await validateStep(lastStep.id, 'stock/processing_validation')
    ...
    const url = router.currentRoute.value.query.child ? '/stock/de_stem_child' : '/stock/de_stem'
...
const rightButtons = configRightButtons({ actions: { save }, dataToValidate, rule: 'genericProcessing' })
