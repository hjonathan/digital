...
const save = async () => {
    ...
    const responseLastStep = await validateStep(lastStep.id, 'stock/debud_validation')
    ...
    const url = router.currentRoute.value.query.child ? '/stock/debud_child' : '/stock/debud'
...
const rightButtons = configRightButtons({ actions: { save }, dataToValidate, rule: 'saveDebud' })
...
onMounted(async () => {
    ...
    if (response.success) {
        ...
        if (itemType === 'harvest-wt') {
            steps.value.push({
                id: 'items_result_1',
                name: t('Wet Weight'),
                subtitle: t('Set wet weight'),
                status: 'current',
                prefix: 'Wet Weight',
                complete: true,
                optional: false,
                ...
                locationSlug: '',
                messageInfo: t('Introduce Wet Weight'),
                component: markRaw(StepWithContainer),
                errors: [],
                next: async (goNext) => {
                    const response = await validateStep('items_result_1', 'stock/debud_validation')
        ...
        if (itemType === 'hang-wt') {
            steps.value.push({
                id: 'items_result_1',
                name: t('Dry Weight'),
                subtitle: t('Introduce Dry Weight'),
                status: 'current',
                prefix: 'Dry weight',
                complete: true,
                optional: false,
            ...
            steps.value.push({
                ...
                errors: [],
                next: async (goNext) => {
                    const response = await validateStep('items_result_2', 'stock/debud_validation')
        ...
        steps.value = steps.value.concat([
            {
                ...
                errors: [],
                next: async (goNext) => {
                    const response = await validateStep('waste', 'stock/debud_validation')
