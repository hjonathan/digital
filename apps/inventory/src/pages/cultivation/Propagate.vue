...
import { Stepper, Title, Alert, Overlay, ActionButtons } from 'layout'
...
const save = async () => {
    ...
    const url = router.currentRoute.value.query.child ? '/stock/change_to_propagation_child' : '/stock/change_to_propagation'
...
onMounted(async () => {
    ...
    if (response.success) {
        ...
        if (itemType === 'seed') {
            steps.value[0].name = t('Seed')

            steps.value[0].prefix = t('Seed')

            steps.value[0].subtitle = t('Seed information')

            steps.value[1].subtitle = t('Remaining quantity')
        }

        if (itemType === 'tissue') {
            steps.value[0].name = t('Tissue')

            steps.value[0].prefix = t('Tissue')

            steps.value[0].subtitle = t('Tissue information')

            steps.value[1].subtitle = t('Remaining quantity')
        }

        if (itemType === 'clone') {
            steps.value[0].name = t('Clone')

            steps.value[0].prefix = t('Clone')

            steps.value[0].subtitle = t('Clone information')
